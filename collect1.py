...
def extract_information(text):
    ...
    for field, variations in field_variations.items():
        for variation in variations:
            # Update the regular expression to capture values on the next line
            if field == 'api':
                pattern = re.compile(fr'{variation}[\s]*\n(\d{{2}}-\d{{3}}-\d{{5}})', re.IGNORECASE | re.DOTALL)
            elif field in ['longitude', 'latitude']:
                pattern = re.compile(fr'{variation}[\s]*\n(\d+° \d+\' \d+\.\d+ [WE])', re.IGNORECASE | re.DOTALL)
                ...
                pattern = re.compile(fr'{variation}[\s]*\n(\d{{2}}/\d{{2}}/\d{{4}})', re.IGNORECASE | re.DOTALL)
